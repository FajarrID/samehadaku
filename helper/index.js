...
// = require('github.com/anasrar/Zippy-DL"
module.exports.zippyGetLink = async (u) => {
...
module.exports.getPagination = async (u, host) => {
  try {
...
module.exports.fetchAllAnime = async (u, replace) => {
  const response = await fetch(u);
  const body = await response.text();
  const $ = cheerio.load(body);
  const element = $(".relat");
...
module.exports.getDetailAnime = async (u, replace) => {
  const response = await fetch(u);
  const body = await response.text();
  const $ = cheerio.load(body);
